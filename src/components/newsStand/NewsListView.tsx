...
export function NewsListView({
  ...
  function getActiveMedia(categories: MediaCategory[]) {
    return getCategoryItems(categories)[0]
  }

  return (
    <Tabs
      value={currentCategory}
      onValueChange={(value) => setCurrentCategory(value as CategoryTabValue)}
      className="mt-4"
    >
      <NewsStandContent>
        <div className="overflow-hidden border border-[#D2DAE0] bg-white">
          <TabsList className="flex h-10 w-full rounded-none border-0 bg-[#F5F7F9] p-0">
            {CATEGORY_TABS.map((tab) => {
              ...
              const activeMedia = getActiveMedia(tab.categories)
              ...
              return (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="group h-10 flex-1 rounded-none border-r border-[#D2DAE0] bg-transparent px-3 text-sm font-medium tracking-[-0.01em] last:border-r-0 data-[state=active]:bg-[#7890E7] data-[state=active]:font-bold data-[state=active]:text-white data-[state=inactive]:text-[#5F6E76]"
                >
